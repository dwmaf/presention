 ...
 * * Sidebar Component
 * * ----------------------------------------
 * * Komponen navigasi utama (sidebar) untuk aplikasi admin
 * * Menyediakan akses ke halaman-halaman utama sistem
 *
 * ! Struktur:
 * ! - Header → logo + role user (Super admin)
 * ! - Navigation → list menu
 * ! - Footer → logout
 *
 * ? Kenapa pakai konfigurasi array (menuItems)?
 * ? - Menghindari duplikasi JSX
 * ? - Mudah menambah / menghapus menu
 * ? - Lebih scalable untuk aplikasi besar
 *
 * ! Navigation Behavior:
 * - Menggunakan Inertia Link untuk SPA navigation
 * - Menggunakan method POST untuk logout
 *
 * * Menu Item terdiri dari:
 * * - label → teks menu
 * * - href → route tujuan
 * * - icon → visual icon
 *
 ...
 * - Hover → biru (normal action)
 * - Danger → merah (logout)
 *
 * TODO Improvement Ideas:
 * TODO - Tambahkan collapse sidebar (responsive)
 */
 ...
 * * Icon Component
 * * ----------------------------------------
 * * Wrapper untuk icon agar ukuran & alignment konsisten
 *
 * ? Kenapa perlu?
 * ? - Semua icon punya ukuran & alignment sama
 * ? - Menghindari styling berulang
 */
...
function SidebarItem({ href, label, icon, method, as = "a", danger, active }) {
    /**
     * * SidebarItem Component
     * * ----------------------------------------
     * * Komponen reusable untuk setiap item menu di sidebar
     *
     * @param {string} href
     * * URL tujuan navigasi
     *
     ...
     *
     * @param {string} [method]
     * * HTTP method (digunakan untuk logout → POST)
     *
     * @param {"a" | "button"} [as="a"]
     * * Elemen yang digunakan oleh Inertia Link
     *
     * @param {boolean} [danger]
     * * Menentukan apakah item adalah aksi berbahaya (logout)
     *
     * @param {boolean} [danger]
     * * Menentukan apakah item adalah aksi berbahaya (logout)
     *
     * ! Behavior:
     * - Normal → hover biru
     * - Danger → hover merah (logout)
     */

    const baseStyle =
        "flex items-center px-4 py-4 gap-4 transition-colors rounded-md w-full";

    const normalStyle = "text-gray-400 hover:bg-blue-100 hover:text-blue-700";

    const activeStyle = "bg-blue-100 text-blue-700 font-semibold";

    const dangerStyle = "text-gray-400 hover:bg-red-100 hover:text-red-700";
    ...
    return (
        <li>
            <Link
                ...
                method={method}
                as={as}
                className={`${baseStyle} ${danger ? dangerStyle : active ? activeStyle : normalStyle}`}
            >
                <Icon>{icon}</Icon>
                <p className="pt-1 font-medium">{label}</p>
            </Link>
        </li>
    );
}
...
function Sidebar() {
    ...
    return (
        <aside className="fixed bottom-0 left-0 top-0 z-50 flex w-64 flex-col gap-4 bg-white pt-8 shadow-lg">
            {/* Header */}
            <div className="flex items-center gap-2 rounded-md px-8">
                <img
                ...
                />
                <div>
                    <p className="text-xl font-semibold">UPA PKK</p>
