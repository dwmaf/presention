...
export default function Division({ auth, divisions, allInterns = [] }) {
    ...
    const submit = (e) => {
        e.preventDefault();
        if (isEditMode) {
            put(route('divisions.update', currentDivision.id), { onSuccess: closeModal });
        } else {
    ...
    const assignIntern = (intern) => {
        router.post(route('divisions.assignIntern', detailDivision.id), { intern_id: intern.id }, {
            preserveScroll: true,
            onSuccess: () => { setMemberSearch(''); setShowAddMember(false); },
        });
    };

    const removeIntern = (intern) => {
        router.delete(route('divisions.removeIntern', [detailDivision.id, intern.id]), {
            preserveScroll: true,
        });
    };
    ...
    const memberSuggestions = useMemo(() => {
        ...
        const memberIds = new Set((detailDivision.interns ?? []).map(i => i.id));
    ...
    return (
        ...
        >
            ...
            <div className="py-10">
                <div className="max-w-7xl mx-auto lg:px-48">
                    ...
                    <div className="flex items-start justify-between mb-6">
                        <div>
                            <h1 className='text-2xl font-semibold'>Manajemen Divisi</h1>
                            <p className="text-sm text-gray-500 mt-1">Daftar divisi yang berperan dalam mendukung operasional dan pengembangan UPA PKK UNTAN.</p>
                        </div>
                        <button
                            onClick={() => openModal()}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                        {divisions.map((division) => (
                            <div key={division.id} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col gap-3">
                                {/* Top row: icon + badge */}
                                ...
                                {/* Nama */}
                                <h3 className="text-xl font-bold text-gray-900 leading-snug">{division.nama_divisi}</h3>

                                {/* Deskripsi */}
                                <p className="text-xs text-gray-500 flex-1 line-clamp-3">
                                    {division.deskripsi || <span className="italic text-gray-300">Belum ada deskripsi.</span>}
                                </p>

                                {/* Tombol */}
                                <div className="flex gap-3 mt-2">
                                    <button
                                        onClick={() => { setDetailDivision(division); setShowAddMember(false); setMemberSearch(''); }}
                                    ...
                                    >
                                        <svg width="40" height="40" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <rect width="32" height="32" rx="8" fill="#FEF9C2"/>
                                            <path d="M18.9847 10.0026C19.0961 10.0126 19.1447 10.1486 19.0647 10.2279L13.5181 15.7746C13.4554 15.8373 13.4106 15.9155 13.3881 16.0013L12.7214 18.5546C12.6994 18.639 12.6998 18.7276 12.7227 18.8118C12.7455 18.8959 12.79 18.9727 12.8517 19.0343C12.9133 19.096 12.9901 19.1404 13.0742 19.1633C13.1584 19.1862 13.247 19.1866 13.3314 19.1646L15.8841 18.4979C15.9699 18.4753 16.0481 18.4302 16.1107 18.3673L21.7361 12.7419C21.7537 12.7239 21.7761 12.7112 21.8006 12.7055C21.8252 12.6997 21.8509 12.7012 21.8747 12.7096C21.8985 12.718 21.9193 12.7331 21.9348 12.753C21.9503 12.7729 21.9598 12.7968 21.9621 12.8219C22.1962 15.055 22.1828 17.3071 21.9221 19.5373C21.7734 20.8073 20.7527 21.8039 19.4874 21.9459C17.1698 22.2027 14.831 22.2027 12.5134 21.9459C11.2474 21.8039 10.2267 20.8073 10.0781 19.5373C9.8034 17.1873 9.8034 14.8133 10.0781 12.4633C10.2267 11.1926 11.2474 10.1959 12.5134 10.0546C14.6631 9.8163 16.8315 9.79888 18.9847 10.0026Z" fill="#A65F00"/>
                                            <path d="M19.8819 10.8248C19.8974 10.8093 19.9158 10.797 19.936 10.7886C19.9563 10.7802 19.978 10.7759 19.9999 10.7759C20.0218 10.7759 20.0435 10.7802 20.0638 10.7886C20.084 10.797 20.1024 10.8093 20.1179 10.8248L21.0606 11.7682C21.0917 11.7994 21.1092 11.8417 21.1092 11.8858C21.1092 11.93 21.0917 11.9723 21.0606 12.0035L15.5319 17.5335C15.5109 17.5545 15.4847 17.5694 15.4559 17.5768L14.1799 17.9102C14.1518 17.9175 14.1222 17.9174 14.0942 17.9097C14.0661 17.9021 14.0406 17.8873 14.02 17.8668C13.9995 17.8462 13.9846 17.8206 13.977 17.7926C13.9694 17.7645 13.9693 17.735 13.9766 17.7068L14.3099 16.4308C14.3173 16.4021 14.3323 16.3759 14.3533 16.3548L19.8819 10.8248Z" fill="#A65F00"/>
                                        </svg>
                                    </button>
                                </div>
                            </div>
            ...
            <Modal show={isModalOpen} onClose={closeModal}>
                <form onSubmit={submit} className="p-6 w-[560px]">
                    ...
                    <div className="mb-4">
                        ...
                        />
                        <InputError message={errors.nama_divisi} className="mt-1" />
                    </div>
                    ...
                    <div className="flex justify-end gap-2">
                        <button type="button" onClick={closeModal} className="px-4 py-2 rounded-lg border border-gray-300 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                            Batal
                        </button>
                        <button type="submit" disabled={processing} className="px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 transition disabled:opacity-50">
            ...
            <Modal show={!!detailDivision} onClose={() => setDetailDivision(null)}>
                {detailDivision && (
                    <div className="p-6 flex flex-col h-[450px] w-[560px]">
                        ...
                        <div className="flex items-center justify-between mb-1 shrink-0">
                            <div className="flex items-center gap-3">
                                <PuzzleSmall />
                                <h2 className="text-lg font-bold text-gray-900">{detailDivision.nama_divisi}</h2>
                            </div>
                            <span className="bg-gray-100 text-gray-600 text-xs font-semibold px-3 py-1 rounded-full">
                                {detailDivision.interns_count ?? detailDivision.interns?.length ?? 0} Anggota
                            </span>
                        </div>
                        {detailDivision.deskripsi && (
                            <p className="text-sm text-gray-500 mb-2 mt-1 shrink-0">{detailDivision.deskripsi}</p>
                        )}
                        ...
                        <div className="relative rounded-lg border border-gray-100 overflow-y-auto flex-1 min-h-0 mb-4">
                            ...
                            <table className="w-full text-sm">
                                <thead className="bg-gray-50 sticky top-0 z-10">
                                    <tr>
                                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500">Profil</th>
                                        <th className="px-4 py-2 text-left text-xs font-semibold text-gray-500">Nama</th>
                                        <th className="px-4 py-2 text-right text-xs font-semibold text-gray-500">Aksi</th>
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {detailDivision.interns && detailDivision.interns.length > 0 ? (
                                        detailDivision.interns.map((intern) => (
                                            <tr key={intern.id}>
                                                <td className="px-4 py-2">
                                                    {intern.foto ? (
                                                        <img src={`/${intern.foto}`} alt={intern.name} className="w-8 h-8 rounded-full object-cover object-top" />
                                                    ) : (
                                                        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-400 text-xs">?</div>
                                                    )}
                                                </td>
                                                <td className="px-4 py-2 font-medium text-gray-800">{intern.name}</td>
                                                <td className="px-4 py-2 text-right">
                                                    <button onClick={() => removeIntern(intern)} className="text-red-400 hover:text-red-600 transition">
                                                        <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" viewBox="0 0 24 24" fill="currentColor">
                                        ...
                                        <tr>
                                            <td colSpan={3} className="px-4 py-6 text-center text-gray-400">Belum ada anggota.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                        ...
                        <div className="flex justify-end shrink-0">
                            <button onClick={() => { setDetailDivision(null); setShowAddMember(false); setMemberSearch(''); }} className="px-4 py-2 rounded-lg border border-gray-300 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                                Tutup
                            </button>
                        </div>
                    </div>
            ...
            <Modal show={confirmingDeletion} onClose={closeDeletionModal}>
                <div className="p-6 w-[560px]">
                    <h2 className="text-lg font-bold text-gray-900 mb-2">Hapus Divisi</h2>
                    <p className="text-sm text-gray-600 mb-6">
                        Apakah anda yakin ingin menghapus divisi <strong>{divisionToDelete?.nama_divisi}</strong>? Tindakan ini tidak dapat dibatalkan.
                    </p>
                    <div className="flex justify-end gap-2">
                        <button onClick={closeDeletionModal} className="px-4 py-2 rounded-lg border border-gray-300 text-sm font-semibold text-gray-700 hover:bg-gray-50 transition">
                            Batal
                        </button>
                        <button onClick={deleteDivision} disabled={deleteForm.processing} className="px-4 py-2 rounded-lg bg-red-500 text-white text-sm font-semibold hover:bg-red-600 transition disabled:opacity-50">
                            Hapus
                        </button>
                    </div>
                </div>
